...
class Block:
    ...
    def _update_children_positions(self, position: Tuple[int, int]) -> None:
        ...
        if self.children:
            ...
            for child in self.children:
                child._update_children_positions((positions[index][0],
                                                  positions[index][1]))
                index += 1
    ...
    def smash(self) -> bool:
        ...
        if n < math.exp(-0.25 * self.level):  # subdivide block
            self.colour = None
            x = self.position[0]
            y = self.position[1]
            child0 = Block((x + child_size, y), child_size,
                           COLOUR_LIST[random.randint(0, 3)], self.level + 1,
                           self.max_depth)
            child1 = Block((x, y), child_size,
                           COLOUR_LIST[random.randint(0, 3)], self.level + 1,
                           self.max_depth)
            child2 = Block((x, y + child_size), child_size,
                           COLOUR_LIST[random.randint(0, 3)], self.level + 1,
                           self.max_depth)
            child3 = Block((x + child_size, y + child_size),
                           child_size, COLOUR_LIST[random.randint(0, 3)],
                           self.level + 1, self.max_depth)
            self.children = [child0, child1, child2, child3]
            child0.smash()
            child1.smash()
            child2.smash()
            child3.smash()
        else:  # pick random colour for block
            c = random.randint(0, len(COLOUR_LIST)-1)
            self.colour = COLOUR_LIST[c]
    ...
    def combine(self) -> bool:
        ...
        count = {}
        for colour in [self.children[0].colour, self.children[1].colour,
        ...
        max_colour = ()
        for key in count:
            if count[key] > max_occurrence:
                max_occurrence = count[key]
                max_colour = key
        if len(count.keys()) == 2 and count[max_colour] == 2 or \
                len(count.keys()) == 4:
            return False
